...
_ENV_KEYS = (
    ...
    "LAMBDA_CANDIDATES", "LAMBDA_MAX_ROUNDS", "LAMBDA_DEFICIT_TOL",
    "RELAXED_SPLIT", "SOFT_MERGE_ROUNDS",
    "ROOT_LOCAL_PARTITION", "ROOT_LOCAL_FUSED_SUMMARIES",
    "SPARSE_PREFILL",
)
...
class TestPartitionConfig(_Base):
    ...
    def test_backend_rules(self):
        ...
        with self.assertRaises(PartitionConfigError):
            ...
            config_from_env()
        _set_env(V7_REUSE, RELAXED_SPLIT="1")
        with self.assertRaises(PartitionConfigError):
            config_from_env()
        _set_env(GPU_MERGE, SOFT_MERGE_ROUNDS="1")
        with self.assertRaises(PartitionConfigError):
            config_from_env()

    def test_relaxed_experiment_flags(self):
        _set_env(
            GPU_MERGE,
        ...
        cfg = get_config()
        self.assertTrue(cfg.relaxed_split)
        self.assertEqual((cfg.soft_merge_rounds, cfg.lambda_max_rounds), (2, 2))
        self.assertIn("relaxed_split=True", cfg.describe())
        self.assertIn("soft_merge_rounds=2", cfg.describe())

    def test_fused_summaries_require_root_local_partition(self):
        _set_env(GPU_MERGE, ROOT_LOCAL_FUSED_SUMMARIES="1")
        with self.assertRaises(PartitionConfigError):
            config_from_env()
        _set_env(
            GPU_MERGE,
            MERGE_TARGET_DIVISOR="128",
            ROOT_LOCAL_PARTITION="1",
            ROOT_LOCAL_FUSED_SUMMARIES="1",
        )
        cfg = get_config()
        self.assertTrue(cfg.root_local_fused_summaries)
        self.assertIn("fused_summaries=True", cfg.describe())
...
class TestGpuBackend(_Base):
    ...
    def test_relaxed_split_keeps_dynamic_count_in_fixed_capacity(self):
        cfg = replace(get_config(), relaxed_split=True, lambda_max_rounds=1).validate()
        n = 4096
        part = build_partition_gpu(_scores("zero", n), n, cfg)
        host = part.to_host()
        budget = n // cfg.summary_compression
        self.assertEqual(part.capacity, budget)
        self.assertEqual(int(part.repairs), 0)
        self.assertLess(int(part.num_leaves), budget)
        self.assertTrue(bool(part.status_ok))
        self.assertTrue(bool((part.leaf_len[int(part.num_leaves) :] == 0).all()))
        validate_leaves(host.leaf_start, host.leaf_len, n)

    def test_forced_target_fallback_guarantees_capacity(self):
        from sglang.srt.layers.attention.nsa.adaptive_hisa.partition_gpu import (
            merge_sync_nonoverlap,
        )

        count0, target = 256, 64
        start = torch.arange(count0, dtype=torch.int64) * 4
        length = torch.full((count0,), 4, dtype=torch.int64)
        totals = torch.randn(count0, KEY_DIM, dtype=torch.float64)
        count = torch.tensor(count0, dtype=torch.int64)
        start, length, count, _, merges = merge_sync_nonoverlap(
            start,
            length,
            count,
            totals,
            ...
            force_target_rounds=2,
        )
        self.assertEqual(int(count), target)
        self.assertEqual([int(x) for x in merges], [128, 64])
        validate_leaves(start[:target], length[:target], count0 * 4, allow_arbitrary=True)

    def test_relaxed_soft_merge_exports_fixed_target_capacity(self):
        cfg = replace(
            get_config(),
        ...
        n = 8192
        part = build_partition_gpu(_scores("random", n, seed=41), n, cfg)
        host = part.to_host()
        target = n // cfg.merge_target_divisor
        self.assertEqual(part.capacity, target)
        self.assertLessEqual(int(part.num_leaves), target)
        self.assertEqual(int(part.repairs), 0)
        self.assertTrue(bool(part.status_ok))
        self.assertEqual(part.round_merges.numel(), 5)  # 2 soft + exact + 2 forced
        self.assertTrue(bool((part.leaf_len[int(part.num_leaves) :] == 0).all()))
        validate_leaves(host.leaf_start, host.leaf_len, n, allow_arbitrary=True)
...
class TestKeyMetricCuda(TestKeyMetric):
    ...
    def test_relaxed_soft_merge_graph_replay_matches_eager(self):
        from sglang.srt.layers.attention.nsa.adaptive_hisa.partition_gpu import (
            graphed_builder,
            reset_graph_cache,
        )
        ...
        n = ROOT * 64 + 5
        k, scale = self._to(*_keys(n, seed=51))
        done = n_complete_tokens(n, ROOT)
        values = key_values(k, scale, done)
        eager = build_partition_gpu(values, n, cfg).to_host()
        reset_graph_cache()
        ...
        self.assertIsNotNone(graph)
        replay = graph.build(values, n).to_host()
        self.assertTrue(replay.meta["status_ok"])
        self.assertLessEqual(replay.leaf_len.numel(), done // 128)
        self.assertTrue(torch.equal(eager.leaf_start.cpu(), replay.leaf_start.cpu()))
        self.assertTrue(torch.equal(eager.leaf_len.cpu(), replay.leaf_len.cpu()))
        reset_graph_cache()
...
class TestRawFp8RegisterPath(unittest.TestCase):
    ...
    def test_root_local_partition_covers_each_root_with_two_leaves(self):
        if not torch.cuda.is_available():
            self.skipTest("CUDA required")
        from sglang.srt.layers.attention.nsa.adaptive_hisa.pkey_fp8_builder import (
            build_partition_from_fp8,
        ...
        cfg = get_config()
        n = ROOT * 7
        k, scale = _keys(n, seed=23)
        part = build_partition_from_fp8(k.cuda(), scale.cuda(), n, cfg)
        self.assertEqual(int(part.num_leaves.item()), n // 128)
        self.assertTrue(part.meta["root_local_partition"])
        starts = part.leaf_start.cpu()
        lengths = part.leaf_len.cpu()
        self.assertTrue(
            torch.equal(starts[0::2], torch.arange(0, n, ROOT, dtype=torch.int32))
        )
        self.assertTrue(
            torch.equal(
                lengths[0::2] + lengths[1::2],
                torch.full((n // ROOT,), ROOT, dtype=torch.int32),
            )
        )
        self.assertTrue(torch.equal(starts[1::2], starts[0::2] + lengths[0::2]))
        self.assertTrue(bool(torch.all(lengths % 8 == 0)))
        self.assertTrue(bool(torch.all((lengths >= 8) & (lengths <= ROOT - 8))))

    def test_root_local_fused_summaries_are_close_to_exact_raw_k(self):
        if not torch.cuda.is_available():
            self.skipTest("CUDA required")
        from sglang.srt.layers.attention.nsa.adaptive_hisa.pkey_fp8_builder import (
            build_partition_from_fp8,
        ...
        cfg = replace(
            ...
            merge_target_divisor=128,
            root_local_partition=True,
            root_local_fused_summaries=True,
        ).validate()
        n = ROOT * 7
        k, scale = _keys(n, seed=29)
        k, scale = k.cuda(), scale.cuda()
        part = build_partition_from_fp8(
            k, scale, n, cfg, summary_scale_fmt="ue8m0"
        )
        fused = part.meta["_summary_fp8"]
        fused_scale = part.meta["_summary_scale"]
        self.assertEqual(tuple(fused.shape), (n // 128, 128))
        self.assertEqual(tuple(fused_scale.shape), (n // 128, 1))
        self.assertNotIn("_merge_totals", part.meta)

        totals = leaf_totals_from_fp8(
            k, scale, part.leaf_start, part.leaf_len, n
        )
        exact, exact_scale = summaries_from_totals(
            totals, part.leaf_len, "ue8m0"
        )
        fused_value = fused.float() * fused_scale
        exact_value = exact.float() * exact_scale
        relative = (fused_value - exact_value).abs() / exact_value.abs().clamp_min(1)
        self.assertLess(float(relative.mean()), 0.02)
