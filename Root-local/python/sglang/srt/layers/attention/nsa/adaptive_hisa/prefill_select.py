...
1. coarse   ``[n_q, capacity]`` = fp8_mqa_logits(q, summaries), with no
            adaptive metadata pass over the score matrix
2. select   per row: leaves in descending coarse score until
            ``prefill_candidate_tokens`` raw tokens (the crossing leaf is clipped);
            leaves in ``[0, sink_tokens)`` are promoted while loading metadata;
...
def coarse_leaf_scores(
    ...
    *,
    ...
    query_ends: torch.Tensor | None = None,
) -> torch.Tensor:
    ...
    the sink leaf forced). With ``mask_metadata=False`` this function is a pure
    Tensor-Core scorer: the weighted selector applies padding and sink rules
    while loading each leaf, avoiding two full score-matrix mask passes.
    """
    import deep_gemm

    n_q = q_fp8.shape[0]
    ks = (
        torch.zeros(n_q, dtype=torch.int32, device=q_fp8.device)
        if query_starts is None
        else query_starts
    )
    ke = (
        num_leaves.reshape(1).to(torch.int32).expand(n_q).contiguous()
...
def local_block_leaves(k_fp8, k_scale, n_complete: int, seq_len: int, scale_fmt=None,
    ...
    nb = (L + block - 1) // block
    fp8, scale = block_mean_pooling_triton(
        k_fp8[n_complete:seq_len],
        k_scale[n_complete:seq_len].reshape(-1).to(torch.float32),
        block,
    )
...
def select_candidates_weighted(
    ...
    *,
    ...
    sink: int = 0,
    force_positions: torch.Tensor | None = None,
):
    ...
    leaf index), and one warp writes each selected interval. ``sink`` promotes
    leaves starting before that token to +inf inside the selector, so callers
    need not rewrite the coarse matrix. Leaf tokens come out in logical order
    (fine scoring and Top-K are permutation invariant).
    ...
    _weighted_select_ops().select(
        ...
        *(seg if seg is not None else (None, None, None)), seg_count,
        force_positions,
        budget, n_complete, total, int(sink),
    )
    if intervals:
...
def sparse_prefill_topk(
    ...
    out = sparse_topk_core(
        ...
        block_local=local_mode() == "block" and k_flat is not None,
    )
    _note(
        ("first", layer_id),
...
def sparse_topk_core(
    ...
    weighted = select_mode() == "weighted"
    # HISA-shaped local mode: mean-pool the current chunk into 64-token
    # pseudo-leaves and let them compete with adaptive prefix leaves for the
    # same token budget. Per-row ``query_ends`` hides future blocks, while the
    # selector promotes the block containing the query position.
    ...
    if (
        ...
        and weighted
        and keys.shape[0] + (local_len + LOCAL_BLOCK - 1) // LOCAL_BLOCK
        <= WEIGHTED_SELECT_MAX_LEAVES
    ):
        prefix_capacity = keys.shape[0]
        local_keys, local_scales, local_starts, local_lens = local_block_leaves(
            flat[0], flat[1], n_complete, int(seq_len)
        )
        keys = torch.cat((keys, local_keys), dim=0)
        scales = torch.cat((scales, local_scales), dim=0)
        leaf_start = torch.cat((leaf_start, local_starts), dim=0)
        leaf_len = torch.cat((leaf_len, local_lens), dim=0)
        num_leaves_i32 = torch.full(
            (1,), keys.shape[0], dtype=torch.int32, device=device
        )
        local_blocks = torch.div(
            (ctx - n_complete).clamp_min(1) + LOCAL_BLOCK - 1,
            LOCAL_BLOCK,
            rounding_mode="floor",
        )
        query_ends = (prefix_capacity + local_blocks).to(torch.int32)
        force_positions = ctx
        dense_local = False
    else:
        block_local = False
        num_leaves_i32 = num_leaves.reshape(1).to(torch.int32)
    weighted = weighted and keys.shape[0] <= WEIGHTED_SELECT_MAX_LEAVES
    # Shared by coarse DeepGEMM, flat fine scoring and the dense local GEMM.
    # Previously each stage allocated and cleared its own all-zero ks vector.
    zero_ctx = torch.zeros_like(ctx)
    # Keep the Tensor-Core scorer independent of adaptive metadata on the
    # production weighted path. Padding and sink promotion are fused into the
    # selector's existing leaf-cache pass.
    coarse = coarse_leaf_scores(
        q_fp8,
        weights,
        keys,
        scales,
        ...
        leaf_start,
        int(cfg.sink_tokens),
        mask_metadata=not weighted,
        query_starts=zero_ctx,
    ...
    if dense_local and local_len > 0:
        ...
        topk_lens = ke_loc + budget
    else:
        sparse_extra_local = 0 if block_local else local_len
        sparse_count = torch.full(
            (n_q,), budget + sparse_extra_local, dtype=torch.int32, device=device
        )
    ...
    for r0 in range(0, n_q, step):
        ...
        if dense_local and local_len > 0:
            ...
            cand = select(rows, sparse_extra_local)
            score = _fine_scores(
                q_fp8[rows],
                w_rows,
                cand,
...
def _fine_scores(
    ...
    *,
    ...
    if flat is not None:
        k, s = flat
        if ks_rows is None:
            ks_rows = torch.zeros_like(ctx_rows)
        return block_sparse_mqa_triton(q_rows, k, s, cand, 1, w_rows, ks_rows, ctx_rows)
