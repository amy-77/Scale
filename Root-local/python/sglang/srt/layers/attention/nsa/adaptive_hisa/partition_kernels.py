...
if HAS_TRITON:
    ...
    @triton.jit
    def _root_local_two_leaf_kernel(
        atom_k_ptr,
        atom_scale_ptr,
        start_ptr,
        length_ptr,
        ...
        stride_kn,
        stride_kd,
        ROOT_ATOMS: tl.constexpr,
        ATOM: tl.constexpr,
        DIM: tl.constexpr,
        STORE_SUMMARIES: tl.constexpr,
        ROUND_SCALE: tl.constexpr,
    ):
        """Choose the best two-segment Key-SSE split independently per root."""
        root_id = tl.program_id(0)
        d = tl.arange(0, DIM)
        atom0 = root_id * ROOT_ATOMS

        total = tl.zeros((DIM,), dtype=tl.float32)
        for a in tl.static_range(ROOT_ATOMS):
            q = tl.load(atom_k_ptr + (atom0 + a) * stride_kn + d * stride_kd)
            scale = tl.load(atom_scale_ptr + atom0 + a)
            total += q.to(tl.float32) * scale

        left = tl.zeros((DIM,), dtype=tl.float32)
        best_gain = tl.full((1,), -float("inf"), dtype=tl.float32)
        best_atoms = tl.full((1,), ROOT_ATOMS // 2, dtype=tl.int32)
        # For a two-segment partition, minimizing child SSE is equivalent to
        # maximizing the between-segment Ward gain.
        for a in tl.static_range(ROOT_ATOMS - 1):
            q = tl.load(atom_k_ptr + (atom0 + a) * stride_kn + d * stride_kd)
            scale = tl.load(atom_scale_ptr + atom0 + a)
            left += q.to(tl.float32) * scale
            nl = a + 1
            nr = ROOT_ATOMS - nl
            delta = left / nl - (total - left) / nr
            gain = (ATOM * nl * nr / ROOT_ATOMS) * tl.sum(delta * delta, axis=0)
            better = gain > best_gain
            best_gain = tl.where(better, gain, best_gain)
            best_atoms = tl.where(better, nl, best_atoms)

        root_start = root_id * ROOT_ATOMS * ATOM
        split_len = best_atoms * ATOM
        out = root_id * 2
        one = tl.arange(0, 1)
        tl.store(start_ptr + out + one, root_start)
        tl.store(length_ptr + out + one, split_len)
        tl.store(start_ptr + out + 1 + one, root_start + split_len)
        tl.store(length_ptr + out + 1 + one, ROOT_ATOMS * ATOM - split_len)
        if STORE_SUMMARIES:
            # Reuse HISA's FP8 atom means instead of rescanning raw K.  The
            # selected boundary is known only after the search, so replay the
            # 32 tiny atom rows and directly emit the two production-layout
            # FP8 leaf summaries.
            best_left = tl.zeros((DIM,), dtype=tl.float32)
            for a in tl.static_range(ROOT_ATOMS):
                q = tl.load(atom_k_ptr + (atom0 + a) * stride_kn + d * stride_kd)
                scale = tl.load(atom_scale_ptr + atom0 + a)
                value = q.to(tl.float32) * scale
                best_left += tl.where(a < best_atoms, value, 0.0)

            left_mean = (best_left / best_atoms).to(tl.bfloat16).to(tl.float32)
            right_atoms = ROOT_ATOMS - best_atoms
            right_mean = ((total - best_left) / right_atoms).to(tl.bfloat16).to(tl.float32)
            left_amax = tl.max(tl.abs(left_mean))
            right_amax = tl.max(tl.abs(right_mean))
            left_scale = tl.maximum(left_amax / 448.0, 1.0e-10)
            right_scale = tl.maximum(right_amax / 448.0, 1.0e-10)
            if ROUND_SCALE:
                left_scale = tl.exp2(tl.ceil(tl.log2(left_scale)))
                right_scale = tl.exp2(tl.ceil(tl.log2(right_scale)))
            left_q = tl.minimum(tl.maximum(left_mean / left_scale, -448.0), 448.0)
            right_q = tl.minimum(tl.maximum(right_mean / right_scale, -448.0), 448.0)
            tl.store(
                summary_ptr + out * DIM + d,
                left_q.to(summary_ptr.dtype.element_ty),
            )
            tl.store(
                summary_ptr + (out + 1) * DIM + d,
                right_q.to(summary_ptr.dtype.element_ty),
            )
            tl.store(summary_scale_ptr + out + one, left_scale)
            tl.store(summary_scale_ptr + out + 1 + one, right_scale)
    ...
    def _merge_force_index_kernel(
        ...
        """Pair (0,1), (2,3), ... only while the valid count exceeds capacity."""
        count = tl.load(count_ptr)
        target = tl.load(target_ptr)
        if count <= target:
            tl.store(new_count_ptr, count)
            tl.store(nsel_ptr, tl.zeros((), tl.int64))
            return
        n_sel = tl.minimum(count - target, count // 2)
        j = tl.arange(0, BLOCK)
        jm = j < n_edges + 1
        selected = (j < n_edges) & ((j % 2) == 0) & ((j // 2) < n_sel)
        tl.store(sel_ptr + j, selected.to(tl.uint8), mask=j < n_edges)
        is_right = (j > 0) & (((j - 1) % 2) == 0) & (((j - 1) // 2) < n_sel)
        keep = jm & (j < count) & (is_right == 0)
        pos = tl.cumsum(keep.to(tl.int64), 0) - 1
        tl.store(src_ptr + pos, j.to(tl.int64), mask=keep)
        tl.store(nsel_ptr, n_sel)
...
def merge_round_triton(
    ...
    *,
    ...
    force_target: torch.Tensor | None = None,
    ...
    ``force_target`` bypasses costs and merges deterministic adjacent pairs
    only on overflow, giving a fixed-round device-side hard-cap fallback.
    ...
    if force_target is not None:
        force_target = force_target.to(torch.int64).reshape(1).contiguous()
        threshold = torch.where(count > force_target, ws["inf"], -ws["inf"])
        _merge_force_index_kernel[(1,)](
            count,
            force_target,
            sel,
            src,
...
def root_local_two_leaf_triton(
    k_fp8: torch.Tensor,
    ...
    *,
    ...
    """Return two adaptive contiguous leaves per root without a global scan.

    Atom means are computed by HISA's grouped pooling kernel. One Triton
    program then evaluates every atom-aligned split in one root and keeps the
    split with maximum between-segment Key-SSE gain.
    """
    if not HAS_TRITON or not k_fp8.is_cuda:
        raise RuntimeError("root-local partition requires Triton CUDA")
    if root != 256 or atom != 8 or k_fp8.shape[1] != 128:
        raise ValueError(
            f"root-local partition supports root=256, atom=8, D=128; "
            f"got root={root}, atom={atom}, D={k_fp8.shape[1]}"
        )
    if n_complete % root:
        raise ValueError(f"sealed length {n_complete} is not divisible by root={root}")
    from sglang.srt.layers.attention.nsa.hisa.triton_kernels import (
        block_mean_pooling_triton,
    )

    atom_k, atom_scale = block_mean_pooling_triton(
        k_fp8[:n_complete], k_scale[:n_complete].reshape(-1).to(torch.float32), atom
    )
    n_roots = n_complete // root
    start = torch.empty(n_roots * 2, dtype=torch.int32, device=k_fp8.device)
    length = torch.empty_like(start)
    if return_summaries:
        summary = torch.empty(
            (n_roots * 2, 128), dtype=torch.float8_e4m3fn, device=k_fp8.device
        )
        summary_scale = torch.empty(
            (n_roots * 2, 1), dtype=torch.float32, device=k_fp8.device
        )
    else:
        # Valid pointers are still required by Triton's signature, but the
        # compile-time STORE_SUMMARIES=False branch never dereferences them.
        summary = torch.empty(1, dtype=torch.float8_e4m3fn, device=k_fp8.device)
        summary_scale = torch.empty(1, dtype=torch.float32, device=k_fp8.device)
    _root_local_two_leaf_kernel[(n_roots,)](
        atom_k,
        atom_scale,
        start,
        length,
        ...
        atom_k.stride(0),
        atom_k.stride(1),
        ROOT_ATOMS=root // atom,
        ATOM=atom,
        DIM=128,
        STORE_SUMMARIES=return_summaries,
        ROUND_SCALE=scale_fmt is not None,
        num_warps=4,
        num_stages=2,
    )
    if return_summaries:
        return start, length, summary, summary_scale
    return start, length
...
def warmup_triton_kernels(device, cfg) -> None:
    ...
    for cap in (1024, 2048, 4096, 8192, 16384):
        ...
        merge_round_triton(
            start, length, totals, count, thr, max_merge_len=0, n_tokens=cap,
            workspace=ws, means=means, force_target=count // 2,
        )
