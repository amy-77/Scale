...
The default speed arm is P-key, L/32 split -> L/128 target merge, adaptive
decode, and sparse prefill on every eligible chunk including the final prompt
chunk (no dense-final fallback). Historical L/8 -> L/64 arms remain selectable:
...
#   sparse_prefill_c32d128    root-local adaptive L/128 speed arm (global split32/merge128 remains the fallback)
...
_ARM_BASE = (
    sys.argv[sys.argv.index('--arm') + 1]
    if '--arm' in sys.argv
    else 'sparse_prefill_c32d128'
)
