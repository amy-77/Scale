"""Adaptive-HISA speed-opt snapshot (adaptive_0921_h201) end-to-end queue on h20-9-57.

Docker equivalent of ``run_pkey_target64_jump.py`` for this machine: same env
(P-key, L/8 split -> L/64 target merge, DECODE_CHUNK=64, SKIP_CLEAN,
FUSE_PAGE_TABLE, RAW_FP8_BUILDER) and the same protocol (freeze source,
warm-up method check, 20K smoke, LongBench-v2 401, RULER 32k/128k 364,
comparison against official DSA). Owns only the container named below.
...
PACKAGE = ROOT / 'code/adaptive_0921_h201'
OUT = ROOT / 'data/adaptive_0921_h201_e2e_20260921'
FROZEN = OUT / 'source'
...
OFFICIAL_TODAY = ROOT / 'data/official_dsa_eval_20260921'
IMAGE = 'qyl/sglang-hisa:eval'
MODEL = '/workspace/qyl/models/deepseek-v3.2'
PORT = 31923
CONTEXT_LEN = 163840
ARM = 'speedopt'
NAME = 'adaptive-0921-h201-e2e'
...
def server_cmd():
    return ['python', '-m', 'sglang.launch_server',
            ...
            '--random-seed', '20260919', '--watchdog-timeout', '900']
...
def start_server():
    ...
    argv = ['docker', 'run', '-d', '--name', NAME, '--label', 'qyl.adaptive_0921_h201=e2e',
...
def compare():
    ...
    for bench, fname in (('longbench_v2', 'longbench_predictions.jsonl'), ('ruler', 'ruler_predictions.jsonl')):
        ...
        entry = {'rows_speedopt': len(arm), 'baselines': {}}
        for label, path in (('official_dsa_hist_20260913', OFFICIAL_HIST / fname),
                            ('official_dsa_sglang_hisa_20260921', OFFICIAL_TODAY / fname)):
            base = rows(path)
            ...
            if common:
                ...
                e.update(acc_speedopt=acc(arm, common), acc_baseline=acc(base, common),
                         delta_pp=100 * (acc(arm, common) - acc(base, common)),
                         speedopt_wins=sum(float(arm[k]['score']) > float(base[k]['score']) for k in common),
                ...
                for field in ('length', 'difficulty', 'task'):
                    ...
                    for value in sorted({str(arm[k][field]) for k in common}):
                        ...
                        e['by_' + field][value] = {'n': len(keys), 'acc_speedopt': acc(arm, keys), 'acc_baseline': acc(base, keys)}
