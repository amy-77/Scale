"""P-key (key_sse) end-to-end queue on the h20-1-aligned dpskv32 decode.

Single arm, LongBench-v2 then RULER (32k + 128k, all records). Owns only the
container ``pkey-align-pkey-20260920``. The source tree is frozen into this
directory first so edits to the working tree cannot leak into the running arm.

Decode alignment under test (see source.sha256.json / source_provenance.json):
  fallback_layers=0, sink 64 + tail 256 raw guards, fixed-8 sealed summaries,
  exact budget fill (partial crossing leaf), GPU partition / CUDA graph / fused Top-K.

Server: same image, cmd and env as the production service, except
  --context-length 163840 (RULER 128k prompts reach 131011 tokens + 256 new;
  the official DSA baseline also ran at the model default 163840),
  SGLANG_NSA_ADAPTIVE_HISA_PARTITION_METRIC=key_sse, port 31921.
"""
import datetime
import fcntl
import hashlib
...
OUT = Path('/DATA/disk0/qyl/data/pkey_align_e2e_20260920')
...
CONTEXT_LEN = 163840
ARM, METRIC = 'pkey', 'key_sse'
NAME = f'pkey-align-{ARM}-20260920'
ACTIVE = None
...
def start_server(info):
    ...
    argv = ['docker', 'run', '-d', '--name', NAME, '--label', 'qyl.pkey_align=20260920',
    ...
    event('SERVER_START', arm=ARM, name=NAME, metric=METRIC, context_len=CONTEXT_LEN)
    sh(argv)
    ACTIVE = NAME
    wait_ready(NAME, PORT)
    for _ in range(60):
