...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    # Number of tokens generated by each request.  Unlike ``seq_lens`` this
    # excludes the prompt and prefix-cache contribution, so attention-policy
    # transitions can be expressed in decode-token space exactly.
    generated_lens_cpu: Optional[List[int]] = None
    ...
    def get_model_worker_batch(
        ...
        return ModelWorkerBatch(
            ...
            generated_lens_cpu=[len(req.output_ids) for req in self.reqs],
...
class ModelWorkerBatch:
    ...
    # Per-request generated-token counts (prompt excluded).  Kept on CPU: it
    # is only used for inexpensive batch-level attention-policy dispatch.
    generated_lens_cpu: Optional[List[int]]
