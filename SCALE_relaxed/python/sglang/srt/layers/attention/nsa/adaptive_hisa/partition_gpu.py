...
def merge_sync_nonoverlap(
    ...
    *,
    ...
    target 模式可先执行固定阈值的 ``soft_rounds``，再执行精确 target
    rounds，最后用固定轮数的确定性 pair merge 处理容量溢出。所有判断都
    留在 device 上，固定 shape、固定 launch 序列且无 host sync。
    """
    ...
    if _pk.use_triton(start) and capacity >= 2:
        ...
        if target is not None:
            for _ in range(int(soft_rounds)):
                # Once the coarse DP is already within capacity, do not merge
                # it further merely because soft rounds are configured.
                soft_threshold = torch.where(
                    count > target,
                    threshold,
                    torch.full_like(threshold, float("-inf")),
                )
                start, length, totals, means, count, n_sel = _pk.merge_round_triton(
                    start, length, totals, count, soft_threshold,
        ...
        for _ in range(int(rounds)):
            ...
            per_round.append(n_sel)
        if target is not None:
            for _ in range(int(force_target_rounds)):
                start, length, totals, means, count, n_sel = _pk.merge_round_triton(
                    start,
                    length,
                    totals,
    ...
    phases = (
        (["soft"] * int(soft_rounds) if target is not None else [])
        + [("target" if target is not None else "threshold")] * int(rounds)
        + (["force"] * int(force_target_rounds) if target is not None else [])
    )
    for phase in phases:
        valid = rows < count
        if phase == "force":
            deficit = merge_deficit(count, target, capacity - 1)
            n_force = torch.minimum(deficit, count.to(torch.int64) // 2)
            selected = (
                valid[:-1]
                & valid[1:]
                & (edge_pos % 2 == 0)
            ...
            if phase == "target":
                thr2 = kth_cost_threshold(
                    torch.where(selected, cost, torch.full_like(cost, float("inf"))),
                    deficit,
                )
                selected &= cost < thr2
        false1 = torch.zeros(1, dtype=torch.bool, device=device)
...
class GpuPartition:
    ...
    status_ok: torch.Tensor  # bool scalar: valid partition fits exported capacity
    method: str
    ...
    def to_host(self) -> PrefillPartition:
        ...
        if meta.get("merge_count"):
            meta.setdefault("premerge_M", self.capacity)
...
def build_partition_gpu(
    ...
    *,
    ...
    if cfg.relaxed_split:
        method += "-relaxed"
    if cfg.soft_merge_rounds:
        method += f"-soft{cfg.soft_merge_rounds}"
    base_meta = {
        ...
        "soft_merge_rounds": cfg.soft_merge_rounds,
        "relaxed_split": cfg.relaxed_split,
    ...
    if cfg.relaxed_split:
        repairs = torch.zeros((), dtype=torch.int64, device=device)
        passes = 0
    else:
    ...
    status_ok = num_leaves <= budget if cfg.relaxed_split else num_leaves == budget
    ...
    force_target_rounds = 0
    if merge_target and cfg.soft_merge_rounds:
        capacity_ratio = -(-budget // merge_target)
        force_target_rounds = max(0, (capacity_ratio - 1).bit_length())
    if cfg.merge_policy == "sync_nonoverlap" and merge_rounds > 0:
        ...
        start, length, count, _totals, round_merges = merge_sync_nonoverlap(
            ...
            soft_rounds=cfg.soft_merge_rounds,
            force_target_rounds=force_target_rounds,
        )
        if target is not None:
            status_ok = status_ok & (count <= target)
