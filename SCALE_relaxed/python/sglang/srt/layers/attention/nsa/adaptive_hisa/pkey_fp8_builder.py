...
def build_partition_from_fp8(
    ...
    *,
    profile: bool = False,
    tree_cache: tuple[torch.Tensor, int] | None = None,
    summary_scale_fmt: str | None = None,
):
    ...
    if cfg.root_local_partition:
        from sglang.srt.layers.attention.nsa.adaptive_hisa import (
            partition_kernels as _pk,
        )

        t0 = _mark(device, profile)
        root_result = _pk.root_local_two_leaf_triton(
            k_fp8,
            k_scale,
            done,
            root=cfg.root,
            # Eight-token boundaries keep the root-local search small and
            # match the minimum practical adaptive leaf granularity.
            atom=8,
            return_summaries=cfg.root_local_fused_summaries,
            scale_fmt=summary_scale_fmt,
        )
        t1 = _mark(device, profile)
        if cfg.root_local_fused_summaries:
            start, length, summary_fp8, summary_scale = root_result
            totals = None
            t2 = t1
        else:
            start, length = root_result
            totals = leaf_totals_from_fp8(k_fp8, k_scale, start, length, done)
            t2 = _mark(device, profile)
        count = torch.full((), start.numel(), dtype=torch.int64, device=device)
        ...
        meta.update(
            {
                ...
                "root_local_fused_summaries": cfg.root_local_fused_summaries,
                "premerge_M": budget,
                "storage_M": start.numel(),
            }
        )
        if totals is not None:
            meta["_merge_totals"] = totals
        else:
            meta["_summary_fp8"] = summary_fp8
            meta["_summary_scale"] = summary_scale
        return GpuPartition(
            n_tokens,
            done,
            start.numel(),
    ...
    t2 = _mark(device, profile)
    # 默认 repair 到恰好 budget；relaxed 实验保留 λ-DP 的动态有效行数。
    if cfg.relaxed_split:
