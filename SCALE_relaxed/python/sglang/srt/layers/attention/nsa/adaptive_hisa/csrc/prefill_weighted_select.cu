 ...
 * weighted_select.cu, batched over rows. Sink leaves are promoted to +inf
 * while caching their keys here. In block-local mode the pseudo-leaf
 * containing each query position is promoted in the same pass.
 *
...
// ``ordered_float_key(+inf)``. Weighted sparse prefill forces sink leaves to
// the front inside this selector so the coarse Tensor-Core scorer can remain a
// metadata-free GEMM (and avoids two full score-matrix mask passes).
constexpr uint32_t kPositiveInfKey = 0xff800000u;
...
__global__ __launch_bounds__(kThreads) void batched_weighted_select_kernel(
    ...
    const int32_t *__restrict__ force_positions,
    ...
    int budget, int n_complete, int total, int sink) {
  ...
  const int32_t force_pos =
      force_positions != nullptr ? force_positions[row0 + r] - 1 : -1;

  for (int i = tid; i < n; i += kThreads) {
    const int32_t len = lengths[i];
    s_len[i] = max(len, 0);
    const bool force_sink = len > 0 && sink > 0 && starts[i] < sink;
    const bool force_query =
        len > 0 && force_pos >= n_complete && starts[i] <= force_pos &&
        force_pos < starts[i] + len;
    s_key[i] = len > 0
                   ? ((force_sink || force_query)
                          ? kPositiveInfKey
                          : ordered_float_key(row_scores[i]))
                   : 0u;
  }
  if (tid == 0) {
    threshold_key = 0;
...
void batched_weighted_select_interface(
    ...
    std::optional<at::Tensor> force_positions,
    int64_t budget, int64_t n_complete, int64_t total, int64_t sink) {
  ...
  TORCH_CHECK(sink >= 0, "sink must be non-negative");
  TORCH_CHECK(budget < (1ll << 25) && n_complete >= 0 && n_complete < (1ll << 26),
              "budget / n_complete exceed the packed-scan field widths");
  const int32_t *force_p = nullptr;
  if (force_positions.has_value()) {
    const at::Tensor &fp = *force_positions;
    TORCH_CHECK(fp.is_cuda() && fp.scalar_type() == at::kInt &&
                    fp.is_contiguous() && fp.numel() >= row0 + rows,
                "force_positions must be contiguous CUDA i32 with enough rows");
    force_p = fp.data_ptr<int32_t>();
  }
...
TORCH_LIBRARY(adaptive_hisa_prefill_select, m) {
  m.def("select(Tensor scores, int row0, int rows, Tensor lengths, Tensor starts, "
        ...
        "Tensor? force_positions, int budget, int n_complete, int total, int sink) -> ()");
}
