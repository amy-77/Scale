...
class PartitionConfig:
    ...
    # P1 experiment: keep the <= M0 λ-DP leaves instead of repairing to exact
    # M0. Physical buffers remain M0-sized; only the device-side valid count
    # becomes dynamic.
    ...
    # Cheap threshold/local-match rounds before exact target merge. Exact target
    # and deterministic forced rounds remain device-side capacity fallbacks.
    soft_merge_rounds: int = 0
    ...
    # Experimental root-local GPU partition: independently choose the best
    # atom-aligned two-leaf split inside every 256-token root. This removes
    # global lambda/repair/merge scans while retaining adaptive boundaries.
    root_local_partition: bool = False
    # Emit leaf FP8 summaries directly from HISA's pooled 8-token atoms in the
    # root-local split kernel. This removes the raw-K totals scan and summary
    # requant launch. It is approximate and remains opt-in pending task-level
    # quality validation.
    root_local_fused_summaries: bool = False
    ...
    def validate(self) -> "PartitionConfig":
        ...
        if self.soft_merge_rounds < 0:
            raise PartitionConfigError("soft_merge_rounds must be non-negative")
        ...
        elif self.soft_merge_rounds:
            raise PartitionConfigError("soft_merge_rounds requires merge_target_divisor > 0")
        if (self.relaxed_split or self.soft_merge_rounds) and self.split_backend != "gpu":
            raise PartitionConfigError("relaxed split/soft merge experiments require split_backend=gpu")
        if self.soft_merge_rounds and self.max_merge_len:
            raise PartitionConfigError("soft merge capacity fallback requires max_merge_len=0")
        ...
        if self.root_local_partition:
            if self.split_backend != "gpu":
                raise PartitionConfigError("ROOT_LOCAL_PARTITION requires SPLIT_BACKEND=gpu")
            if self.relaxed_split or self.soft_merge_rounds:
                raise PartitionConfigError(
                    "ROOT_LOCAL_PARTITION is a separate path from relaxed split/soft merge"
                )
            if self.root != 256 or 8 % self.atom or self.merge_target_divisor != 128:
                raise PartitionConfigError(
                    "ROOT_LOCAL_PARTITION currently requires ROOT=256, "
                    "ATOM dividing 8, MERGE_TARGET_DIVISOR=128"
                )
        if self.root_local_fused_summaries and (
            not self.root_local_partition or not self.build_summaries
        ):
            raise PartitionConfigError(
                "ROOT_LOCAL_FUSED_SUMMARIES requires "
                "ROOT_LOCAL_PARTITION=1 and BUILD_SUMMARIES=1"
            )
    ...
    def describe(self) -> str:
        return (
            ...
            f"relaxed_split={self.relaxed_split} "
            ...
            f"soft_merge_rounds={self.soft_merge_rounds} "
            f"summaries={self.build_summaries} "
            f"graph_build={self.graph_build} tree_cache={self.tree_cache} "
            f"root_local={self.root_local_partition} "
            f"fused_summaries={self.root_local_fused_summaries} "
            f"gpu_stream={self.gpu_stream} cpu_overlap={self.cpu_overlap} "
...
def config_from_env() -> PartitionConfig:
    ...
    for name, field in (("ATOM", "atom"), ("ROOT", "root"), ("SUMMARY_COMPRESSION", "summary_compression"),
                        ...
                        ("SOFT_MERGE_ROUNDS", "soft_merge_rounds"),
    ...
    raw = _env("RELAXED_SPLIT")
    if raw is not None:
        updates["relaxed_split"] = _parse_bool("RELAXED_SPLIT", raw)
    ...
    raw = _env("ROOT_LOCAL_PARTITION")
    if raw is not None:
        updates["root_local_partition"] = _parse_bool("ROOT_LOCAL_PARTITION", raw)
    raw = _env("ROOT_LOCAL_FUSED_SUMMARIES")
    if raw is not None:
        updates["root_local_fused_summaries"] = _parse_bool(
            "ROOT_LOCAL_FUSED_SUMMARIES", raw
        )
