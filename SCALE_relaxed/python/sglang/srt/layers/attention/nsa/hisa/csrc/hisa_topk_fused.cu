...
// Adaptive-HISA: reuse the radix selector above verbatim. Only the epilogue
// changes from block_id * K + offset to an explicit candidate token table.
// The optional page-table lookup is fused here as well, without materializing
// relative indices or a separate coordinate-transform tensor.
__global__ __launch_bounds__(kThreadsPerBlock)
void topk_candidates_kernel(
    const FastTopKParams params, const int32_t *__restrict__ candidates,
    int64_t candidate_stride, const int32_t *__restrict__ seq_lens,
    const int32_t *__restrict__ page_table, int64_t page_stride,
    int64_t page_cols, int32_t *__restrict__ output) {
  ...
  for (int i = tid; i < TopK; i += kThreadsPerBlock) {
    ...
    const int raw = r >= 0 && r < length
        ? candidates[bid * candidate_stride + r] : -1;
    int value = raw >= 0 && raw < seq_lens[bid] ? raw : -1;
    if (page_table != nullptr)
      value = value >= 0 && value < page_cols
          ? page_table[bid * page_stride + value] : -1;
    output[bid * TopK + i] = value;
  }
}
...
void topk_candidates_interface(
    const at::Tensor &score, const at::Tensor &lengths,
    const at::Tensor &candidates, const at::Tensor &seq_lens,
    std::optional<at::Tensor> page_table, at::Tensor &output) {
  CHECK_CUDA(score);
  CHECK_CUDA(lengths);
  ...
  TORCH_CHECK(candidates.dim() == 2 && candidates.is_contiguous());
  TORCH_CHECK(candidates.size(0) == B && candidates.size(1) == score.size(1));
  TORCH_CHECK(seq_lens.dim() == 1 && seq_lens.is_contiguous() && seq_lens.size(0) == B);
  TORCH_CHECK(output.dim() == 2 && output.is_contiguous() &&
              output.size(0) == B && output.size(1) == TopK);
  const int32_t *pages = nullptr;
  int64_t page_stride = 0, page_cols = 0;
  if (page_table.has_value()) {
    const auto &table = page_table.value();
    CHECK_CUDA(table);
    TORCH_CHECK(table.dim() == 2 && table.stride(1) == 1 && table.size(0) == B);
    pages = table.data_ptr<int32_t>();
    page_stride = table.stride(0);
    page_cols = table.size(1);
  }
  setup_kernel_smem_once<topk_candidates_kernel, kSmem>();
  topk_candidates_kernel<<<dim3(B), dim3(kThreadsPerBlock), kSmem,
                           at::cuda::getCurrentCUDAStream().stream()>>>(
      params, candidates.data_ptr<int32_t>(), candidates.stride(0),
      seq_lens.data_ptr<int32_t>(), pages, page_stride, page_cols,
