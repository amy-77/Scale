...
# partition-like leaves: random lengths avg 64 covering n_complete
n = n_complete // 64
...
coarse = ps.coarse_leaf_scores(q, w, keys, scales, nl, lens, starts, 64)
t_coarse = timeit(lambda: ps.coarse_leaf_scores(q, w, keys, scales, nl, lens, starts, 64))
cands = [ps.select_candidates_weighted(coarse, starts, lens, nl, slice(r0, r0 + step), budget, n_complete, 0) for r0 in range(0, n_q, step)]
t_select = timeit(lambda: [ps.select_candidates_weighted(coarse, starts, lens, nl, slice(r0, r0 + step), budget, n_complete, 0) for r0 in range(0, n_q, step)])
print(f"coarse GEMM [8192 x {n}]: {t_coarse:.3f} ms;  weighted select: {t_select:.3f} ms")
