...
def _sparse_paged_mqa_grouped_kernel(
    ...
    # K=1 candidate tables may contain -1 padding. Triton's signed // and %
    # truncate toward zero, so token=-1 can still produce logical_page=0
    # with row=-1. Mask the token BEFORE the raw-key load, not just logits.
    valid_per_row = (token_abs >= 0) & valid_page & (phys_per_row >= 0) & (phys_per_row < num_phys)
...
def sparse_paged_mqa_triton(
    ...
    K=1 accepts a token-ID candidate table, including -1 padding, for
    Adaptive-HISA. It uses the SAME grouped kernel and paged key layout.

    Unified grouped dispatch for K∈{1,8,16,32,64,128}. The grouped kernel's
