...
def hisa_topk_candidates_fused(score, candidates, lengths, seq_lens, page_table=None):
    """Adaptive leaf/token-ID variant of the HISA radix + mapping kernel.

    With page_table=None return request-relative token IDs. Otherwise map
    directly to physical token locations in the same CUDA kernel.
    """
    out = torch.empty((score.shape[0], TOPK), dtype=torch.int32, device=score.device)
    torch.ops.hisa_topk_fused.topk_candidates(
        score, lengths, candidates, seq_lens.reshape(-1), page_table, out
    )
    return out
